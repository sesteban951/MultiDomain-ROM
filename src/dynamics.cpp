...
// NonLinear Dynamics function, xdot = f(x, u, d)
Dynamics_Result_3D Dynamics::dynamics_3D(const Vector_12d& x_sys, const Vector_6d& u, const Vector_6d& p_feet, const Domain& d) 
{
    // want to compute the state derivative
...
// compute the leg state
Vector_12d Dynamics::compute_leg_state_3D(const Vector_12d& x_sys, const Vector_6d& u, const Vector_6d& p_feet, const Domain& d)
{
    // want to compute the leg states
    Vector_12d x_legs;
    Vector_6d x_leg;
...
// compute foot state in world frame
Vector_12d Dynamics::compute_foot_state_3D(const Vector_12d& x_sys, const Vector_12d& x_legs, const Vector_6d& p_feet, const Domain& d)
{
    // want to compute the foot states
    Vector_12d x_feet;
    Vector_6d x_foot;
...
// Touch-Down (TD) Switching Surface -- checks individual legs
bool Dynamics::S_TD_3D(Vector_12d x_feet, Leg_Idx leg_idx) 
{   
    // foot variable
...
// Take-Off (TO) Switching Surface -- checks individual legs
bool Dynamics::S_TO_3D(Vector_12d x_sys, Vector_12d x_legs, Vector_6d u, Leg_Idx leg_idx) 
{    
    // leg state
...
// interpolate an input signal
Vector_6d Dynamics::interpolate_control_input_3D(double t, const Vector_1d_Traj& T_u, const Vector_6d_Traj& U) 
{
    // we want to find the interpolated control input
    Vector_6d u;
...
// resize the solution bundle to the same as the time vector
void Dynamics::resizeSolution_3D(Solution_3D& sol, const Vector_1d_Traj& T_x) {
    const int N = T_x.size();
    sol.x_sys_t.resize(N);
    sol.x_leg_t.resize(N);
    sol.x_foot_t.resize(N);
...
// RK3 Integration of the system dynamics
Solution_3D Dynamics::RK3_rollout_3D(const Vector_1d_Traj& T_x, const Vector_1d_Traj& T_u, 
                                     ...
                                     const Vector_6d_Traj& U) 
{
    ...
    Solution_3D sol;
    this->resizeSolution_3D(sol, T_x);

    // initial condition
    Vector_12d x0_legs;
    Vector_12d x0_feet;
    x0_legs = this->compute_leg_state_3D(x0_sys, U[0], p0_feet, d0);
    x0_feet = this->compute_foot_state_3D(x0_sys, x0_legs, p0_feet, d0);

    // first iteration just to get the initial leg forces and torques
    Vector_6d lambda0;
    Vector_6d tau0;
    Dynamics_Result_3D res = this->dynamics_3D(x0_sys, U[0], p0_feet, d0);
    lambda0 = res.lambdas;
    tau0 = res.taus;
    ...
    for (int k = 1; k < N; k++) {
        ...
        // interpolate the control input
        u1 = this->interpolate_control_input_3D(t1, T_u, U);
        u2 = this->interpolate_control_input_3D(t2, T_u, U);
        u3 = this->interpolate_control_input_3D(t3, T_u, U);

        // vector fields for RK3 integration
        res1 = this->dynamics_3D(xk_sys, 
                              u1, p_feet, dk);
        f1 = res1.xdot;
        res2 = this->dynamics_3D(xk_sys + 0.5 * dt * f1,
                              u2, p_feet, dk);
        f2 = res2.xdot;
        res3 = this->dynamics_3D(xk_sys - dt * f1 + 2.0 * dt * f2,
                              u3, p_feet, dk);
        f3 = res3.xdot;

        // take the RK3 step
        xk_sys = xk_sys + (dt / 6.0) * (f1 + 4.0 * f2 + f3);
        xk_legs = this->compute_leg_state_3D(xk_sys, u3, p_feet, dk);
